  ...
  for(dest_pos = 0; dest_pos < dest_count && src_pos < src_size; )
  {
    ...
    if(pack_byte >= 0x80)
    {
      ...
      for(i = 0; i < num; i++, dest_pos++)
        *(dest += dest_pitch) = copy;
    }
    else
    {
      /* Literal */
      num = pack_byte + 1;
      if(src_pos + num > src_size)
        break;

      for(i = 0; i < num; i++, dest_pos++)
        *(dest += dest_pitch) = src[src_pos++];
    }
  }
...
static enum icns_error icns_image_prepare_png_or_argb_for_icns(
 struct icns_data * RESTRICT icns, struct icns_image * RESTRICT image, size_t *sz)
{
  const struct icns_format *format = image->format;
  bool is_alpha = (format->type == ICNS_ARGB_OR_PNG);
...
/* Load internal or iconset image that is always packed RGB. */
 ...
 struct icns_data * RESTRICT icns, struct icns_image * RESTRICT image, size_t sz)
{
  ...
  if(IMAGE_IS_JPEG_2000(image) || IMAGE_IS_PNG(image))
  {
...
const struct icns_format icns_format_icp4 =
{
  ...
  16, 16, 1,
  0x1070,
  icns_image_prepare_png_or_argb_for_icns,
